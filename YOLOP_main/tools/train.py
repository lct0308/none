...
def main():
    ...
    if rank in [-1, 0]:
        ...
        if cfg.TRAIN.ENC_LANE_ONLY:    # Only train encoder and detection branchs
            logger.info('freeze Da_Seg head and Det head...')
            for k, v in model.named_parameters():
                v.requires_grad = True  # train all layers
        ...
        if cfg.TRAIN.ENC_DET_LANE:    # Only train encoder and detection branchs
            logger.info('freeze Da_Seg head...')
            for k, v in model.named_parameters():
                v.requires_grad = True  # train all layers
