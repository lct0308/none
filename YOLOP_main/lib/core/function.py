import time
from YOLOP_main.lib.core.evaluate import ConfusionMatrix,SegmentationMetric
from YOLOP_main.lib.core.general import non_max_suppression,check_img_size,scale_coords,xyxy2xywh,xywh2xyxy,box_iou,coco80_to_coco91_class,plot_images,ap_per_class,output_to_target
from YOLOP_main.lib.utils.utils import time_synchronized
from YOLOP_main.lib.utils import plot_img_and_mask,plot_one_box,show_seg_result
...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        with amp.autocast(enabled=device.type != 'cpu'):
            outputs = model(input)
            total_loss, head_losses = criterion([outputs[0], outputs[2]], target, shapes,model)
            # print(head_losses)
        
        # print(f"total_loss: {total_loss}")
        # print(f"det_loss: {head_losses[0] + head_losses[1] + head_losses[2]}")
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    batch_size = config.TRAIN.BATCH_SIZE_PER_GPU * (len(config.GPUS) if type(config.GPUS)!=int else 1)
    test_batch_size = config.TEST.BATCH_SIZE_PER_GPU * (len(config.GPUS) if type(config.GPUS)!=int else 1)
    training = False
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            if config.TEST.PLOTS:
                ...
                # if batch_i >= 0:
                    for i in range(test_batch_size):
                        ...
                        # plot_img_and_mask(img_test, seg_mask, i,epoch,save_dir)
                        img_ll1 = img_ll.copy()
                        img_det = show_seg_result(img_ll, ll_seg_mask, i,epoch,save_dir, is_ll=True)
                        img_gt = show_seg_result(img_ll1, ll_gt_mask, i, epoch, save_dir, is_ll=True, is_gt=True)

                        # img_det = cv2.imread(paths[i])
                        # img_gt = img_det.copy()
                        det = output[i].clone()
                        pred0 = output[i].clone()
                        if len(det):
                            # print(f"img[i].shape[1:]: {img[i].shape[1:]}, img_det.shape: {img_det.shape}")
                            det[:,:4] = scale_coords(img[i].shape[1:],det[:,:4],img_det.shape).round()
                            pred0[:, :4] = scale_coords(img[i].shape[1:], pred0[:, :4], img_det.shape).round()
                            print(max(det[:,4]))
                        for *xyxy, conf, cls in pred0.tolist():
                            # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()
                            if xywh[2] < 25 or xywh[3] < 25:
                                false_row = torch.tensor([*xyxy, conf, cls]).to(device)
                                det = det[~torch.all(det == false_row, dim=1)]
                        for *xyxy,conf,cls in reversed(det):
                            #print(cls)
                            label_det_pred = f'{names[int(cls)]} {conf:.2f}'
                            plot_one_box(xyxy, img_det , label=label_det_pred, color=colors[int(cls)], line_thickness=3)
                        cv2.imwrite(save_dir+"/batch_{}_{}_pred.png".format(epoch,i),img_det)
        ...
        for si, pred in enumerate(output):
            ...
            scale_coords(img[si].shape[1:], pred0[:, :4], shapes[si][0], shapes[si][1])
            ...
            for *xyxy, conf, cls in pred0.tolist():
                ...
                if xywh[2] < 25 or xywh[3] < 25:
                    false_row = torch.tensor([*xyxy, conf, cls]).to(device)
                    predn = predn[~torch.all(predn == false_row, dim=1)]
            ...
            if config.TEST.SAVE_TXT:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    txt_path = os.path.join(txt_path, (path.stem + '.txt'))
    ...
    if len(stats) and stats[0].any():
        ...
        mp, mr, map50, map70, map75, map, f1 = p.mean(), r.mean(), ap50.mean(), ap70.mean(),ap75.mean(),ap.mean(),f1.mean()
        # print('p:{},mp:{}'.format(p, mp))
    ...
    detect_result = np.asarray([mp, mr, map50, map, f1])
