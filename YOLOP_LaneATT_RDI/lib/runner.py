...
class Runner:
    ...
    def train(self):
        ...
        for epoch in trange(starting_epoch, max_epochs + 1, initial=starting_epoch - 1, total=max_epochs):
            ...
            for i, (images, labels, _, rdi) in enumerate(pbar):
                images = images.to(self.device)
                labels = labels.to(self.device)
                rdi = rdi.to(self.device)

                # Forward pass
                outputs = model(images, rdi, **self.cfg.get_train_parameters())
    ...
    def eval(self, epoch, on_val=False, save_predictions=False):
        ...
        with torch.no_grad():
            for idx, (images, _, _, rdi) in enumerate(tqdm(dataloader)):
                # print(len(dataloader))
                images = images.to(self.device)
                rdi = rdi.to(self.device)
                output = model(images, rdi, **test_parameters)
