...
class LaneDataset(Dataset):
    def __init__(self,
        ...
        if dataset == 'tusimple':
            ...
            self.dataset = ONCELane(**kwargs)
        elif dataset == 'mydataset':
            self.dataset = MyDataset(**kwargs)
        else:
    ...
    def __getitem__(self, idx):
        ...
        img_path = item['path'] # "/data1/pbw/Mydataset/images/2023-04-07-21-24-07/1680873889.478629.png"
        lidar_path = img_path.replace('images', 'rdi')
        # print(lidar_path)
        rdi_org = cv2.imread(lidar_path, 0)
        # rdi = cv2.cvtColor(ori_rdi, cv2.COLOR_GRAY2RGB)
        ...
        # print(line_strings_org.shape)
        for i in range(30):
            rdi, line_strings = self.transform(image=rdi_org.copy(), line_strings=line_strings_org)
            # img, rdi, line_strings = self.transform(image=img_org.copy(), rdi=rdi_org.copy(), line_strings=line_strings_org)
