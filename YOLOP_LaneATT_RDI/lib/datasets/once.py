...
class ONCELane(LaneDatasetLoader):
    def __init__(self, max_lanes=None, split='train', root=None, official_metric=True):
        ...
        self.anno_root = os.path.join(root, 'gt_2d')
        self.image_root = os.path.join(root, "data")
        self.list = os.path.join(self.anno_root, SPLIT_FILES[split])

        self.img_w, self.img_h = 1920, 1020
        self.annotations = []
    ...
    def load_annotation(self, json_path):
        ...
        with open(json_path, 'r') as file:
            ...
            img_name = info_dict['file_path'].split('/',5)[5]
            image_path = os.path.join(self.image_root, img_name)
            lanes1 = info_dict['lane_lines']
            for idx in range(len(lanes1)):
                ori_lane = lanes1[idx]
                uv1 = np.array(ori_lane['uv']).T * np.array([4, 1020/360])
                uv1 = uv1.T
    ...
    def load_annotations(self):
        ...
        cache_path = 'cache/once_{}.json'.format(self.split)

        if os.path.exists(cache_path):
            self.logger.info('Loading ONCE annotations (cached)...')
            with open(cache_path, 'r') as cache_file:
                data = json.load(cache_file)
                self.annotations = data['annotations']
                self.max_lanes = data['max_lanes']
        else:
            self.logger.info('Loading ONCE annotations and caching...')
            with open(self.list, 'r') as list_file:
                for file in tqdm(list_file):
                    file = file.split()
                    json_line = file[0].split('/', 1)[1]
                    json_path = os.path.join(self.anno_root, json_line)
                    anno = self.load_annotation(json_path)
                    org_path = os.path.join('data', json_line)
                    anno['org_path'] = org_path.replace('json', 'jpg')
    ...
    def eval_predictions(self, predictions, output_basedir):
        ...
        for idx, pred in enumerate(tqdm(predictions)):
            ...
            output_filename = os.path.basename(self.annotations[idx]['old_anno']['org_path']).replace(".jpg", ".json")
