...
from YOLOP_LaneATT_RDI.lib.models.common import Conv, SPP, Bottleneck, BottleneckCSP, Focus, Concat, Detect, SharpenConv
from torch.nn import Upsample

from nms import nms
from YOLOP_LaneATT_RDI.lib.lane import Lane
from YOLOP_LaneATT_RDI.lib.focal_loss import FocalLoss
...
class MCnet(nn.Module):
    def __init__(self, block_cfg):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=3,out_channels=2,kernel_size=1,stride=1,padding=0)
    ...
    def forward(self, x):
        cache = []
        # print('image:',x.shape, '  lidar:',lidar.shape)
...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        # Some definitions
        self.feature_extractor = MCnet(YOLOP)
        backbone_nb_channels, self.stride = 512, 32
        self.img_w = img_w
        # print(img_w)
        # print(img_h)
        self.n_strips = S - 1
        ...
        self.conv1 = nn.Conv2d(backbone_nb_channels, self.anchor_feat_channels - 1, kernel_size=1)
        # self.conv2 = nn.Conv2d(self.anchor_feat_channels, self.anchor_feat_channels, kernel_size=1)
    ...
    def forward(self, x, rdi):
        conf_threshold=0.2
        nms_thres=30
        nms_topk=7
        # print(x.shape)
        # x = torch.cat([x, rdi], dim=1)
        batch_features = self.feature_extractor(x)
        # print(batch_features.shape)
        batch_features = self.conv1(batch_features)
        # print(batch_features.shape)
        # print(rdi.shape)
        batch_features = torch.cat([batch_features, rdi], dim=1)
    ...
    def loss(self, proposals_list, targets, cls_loss_weight=10):
        ...
        for (proposals, anchors, _, _), target in zip(proposals_list, targets):
            ...
            with torch.no_grad():
                ...
                # print(f"invalid_offsets_mask device: {invalid_offsets_mask.device}")
                # print(f"all_indices device: {all_indices.device}")
                # print(f"ends device: {ends.device}")
